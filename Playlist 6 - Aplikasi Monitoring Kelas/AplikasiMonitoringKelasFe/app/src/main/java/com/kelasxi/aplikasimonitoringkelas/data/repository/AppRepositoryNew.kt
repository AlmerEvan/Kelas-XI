...
class AppRepositoryNew(private val apiService: ApiService) {
    ...
    suspend fun createSchedule(token: String, request: ScheduleRequest): Result<Schedule> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.createSchedule("Bearer $token", request)
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val errorBody = response.errorBody()?.string()
                    Result.failure(Exception("Gagal membuat jadwal: ${response.code()} - $errorBody"))
                }
            } catch (e: Exception) {
                Result.failure(Exception("Gagal terhubung ke server: ${e.message}"))
            }
    ...
    suspend fun getGuruPengganti(token: String, tanggal: String? = null, kelas: String? = null): Result<GuruPenggantiResponse> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                android.util.Log.d("GuruPenggantiAPI", "Response code: ${response.code()}, isSuccessful: ${response.isSuccessful}")
                
                if (response.isSuccessful && response.body() != null) {
                    val guruPenggantiResponse = response.body()!!
                    android.util.Log.d("GuruPenggantiAPI", "Success: ${guruPenggantiResponse.success}, Data count: ${guruPenggantiResponse.data?.size}")
                    ...
                    android.util.Log.e("GuruPenggantiAPI", errorMessage)
                ...
                android.util.Log.e("GuruPenggantiAPI", "Exception: ${e.message}", e)
                Result.failure(Exception("Gagal terhubung ke server: ${e.message}"))
            }
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Result.success(apiResponse.toResponse())
                } else {
                    val errorBody = response.errorBody()?.string()
    ...
    // ==================== GURU PENGGANTI CONFIRMATION ====================
    
    suspend fun confirmReplacementTeacher(
        token: String,
        guruPenggantiId: Int,
        siswaId: Int
    ): Result<ReplacementConfirmation> {
        return withContext(Dispatchers.IO) {
            try {
                val request = ConfirmReplacementTeacherRequest(
                    guru_pengganti_id = guruPenggantiId,
                    siswa_id = siswaId
                )
                val response = apiService.confirmReplacementTeacher("Bearer $token", request)
                if (response.isSuccessful && response.body() != null) {
                    val apiResponse = response.body()!!
                    if (apiResponse.success && apiResponse.data != null) {
                        Result.success(apiResponse.data)
                    ...
                    val errorBody = response.errorBody()?.string()
                    Result.failure(Exception("Gagal mengkonfirmasi guru pengganti: ${response.code()} - $errorBody"))
                }
            } catch (e: Exception) {
                Result.failure(Exception("Gagal terhubung ke server: ${e.message}"))
            }
    ...
    suspend fun getReplacementTeacherReport(
        token: String,
        tanggal: String? = null,
        kelas: String? = null,
        search: String? = null
    ): Result<ReplacementTeacherReportResponse> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getReplacementTeacherReport("Bearer $token", tanggal, kelas, search)
                if (response.isSuccessful && response.body() != null) {
                    val reportResponse = response.body()!!
                    if (reportResponse.success) {
                        Result.success(reportResponse)
                    } else {
                        Result.failure(Exception(reportResponse.message))
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    Result.failure(Exception("Gagal mengambil laporan: ${response.code()} - $errorBody"))
    ...
    suspend fun getGuruPenggantiConfirmations(
        token: String,
        guruPenggantiId: Int
    ): Result<SpecificGuruPenggantiConfirmationResponse> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getGuruPenggantiConfirmations("Bearer $token", guruPenggantiId)
                if (response.isSuccessful && response.body() != null) {
                    val confirmationResponse = response.body()!!
                    if (confirmationResponse.success) {
                        Result.success(confirmationResponse)
                    } else {
                        Result.failure(Exception(confirmationResponse.message))
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    Result.failure(Exception("Gagal mengambil konfirmasi: ${response.code()} - $errorBody"))
                }
            } catch (e: Exception) {
                Result.failure(Exception("Gagal terhubung ke server: ${e.message}"))
            }
